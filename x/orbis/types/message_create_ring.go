package types

import (
	errorsmod "cosmossdk.io/errors"
	sdk "github.com/cosmos/cosmos-sdk/types"
	sdkerrors "github.com/cosmos/cosmos-sdk/types/errors"
)

var _ sdk.Msg = &MsgCreateRing{}

func NewMsgCreateRing(
	creator string,
	peerNodeKeys []string,
	threshold uint32,
	pssInterval uint64,
	policyID string,
	currentVersion uint64,
	allowTrustedAuthRelays bool,
	trustedAuthRelayDIDs []string,
) *MsgCreateRing {
	return &MsgCreateRing{
		Creator:                creator,
		PeerNodeKeys:           peerNodeKeys,
		Threshold:              threshold,
		PssInterval:            pssInterval,
		PolicyId:               policyID,
		CurrentVersion:         currentVersion,
		AllowTrustedAuthRelays: allowTrustedAuthRelays,
		TrustedAuthRelayDids:   trustedAuthRelayDIDs,
	}
}

func (msg *MsgCreateRing) ValidateBasic() error {
	if _, err := sdk.AccAddressFromBech32(msg.Creator); err != nil {
		return errorsmod.Wrapf(sdkerrors.ErrInvalidAddress, "invalid creator address (%s)", err)
	}
	if len(msg.PeerNodeKeys) == 0 {
		return errorsmod.Wrap(ErrInvalidRing, "missing peer_node_keys")
	}
	if msg.Threshold == 0 || int(msg.Threshold) > len(msg.PeerNodeKeys) {
		return errorsmod.Wrapf(ErrInvalidRing, "threshold %d is invalid for committee size %d", msg.Threshold, len(msg.PeerNodeKeys))
	}
	if msg.PolicyId == "" {
		return errorsmod.Wrap(ErrInvalidRing, "missing policy_id")
	}
	if err := ValidatePSSInterval(msg.PssInterval); err != nil {
		return err
	}
	if msg.Reporting != nil {
		if err := ValidateReportingConfig(*msg.Reporting); err != nil {
			return err
		}
	}
	if err := ValidateTrustedAuthRelayConfig(msg.AllowTrustedAuthRelays, msg.TrustedAuthRelayDids); err != nil {
		return err
	}
	return nil
}
