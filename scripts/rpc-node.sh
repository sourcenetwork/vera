...
cp ~/.vera/config/genesis.json $HOME_DIR/config/genesis.json

sedi 's/^minimum-gas-prices = .*/minimum-gas-prices = "0.001uopen,0.001ucredit"/' ~/.vera/config/app.toml

# Enable API / GRPC
sedi 's/^enable = .*/enable = true/' ~/.vera/config/app.toml
...
# NODE_ID=$($BIN tendermint show-node-id --home ~/.vera-full)
