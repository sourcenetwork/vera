...
Example:
	verad testnet start --v 4 --output-dir ./.testnets
	`,
...
Example:
	verad testnet init-files --v 4 --output-dir ./.testnets --starting-ip-address 192.168.10.2
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
	...
	cmd.Flags().String(flagNodeDaemonHome, "verad", "Home directory of the node's daemon configuration")
	cmd.Flags().String(
...
func initGenFiles(
	...
	coreGenesis.ChainConfig.AllowZeroFeeTxs = noFees
	appGenState[coretypes.ModuleName] = clientCtx.Codec.MustMarshalJSON(coreGenesis)
